...
8. **Calibration mode (P1-adjacent).** A practice drill that trains an operator's judgment instead of replacing it, built on top of the Curiosity engine. It deepens the moat, since taste is the one thing that cannot be offloaded to the tool. Effort: M. Dependency: Curiosity engine. Full spec in `Anvil_Calibration_Mode_Spec.md`.
9. **Awareness stage as a first-class input.** Add the buyer's awareness stage (unaware, problem-aware, solution-aware, product-aware, most aware) alongside objective and voice. It sets format, length, which block leads, and how direct the call to action is, so every output gets smarter, not just long-form. Effort: S to M. Dependency: none, but it is the prerequisite for long-form. Full spec in `Anvil_Awareness_and_LongForm_Spec.md`.
10. **Segment builder (WHAT, WHO, WHY).** Replace the single avatar step with a real slicing method: outcomes sorted by want not by type, demographics with buyer first, and facets (belief, identity, state, value, occasion). Enforce the say-it test and the gate, only make a slice that needs a different ad, and allow stacking two facets to find a pocket nobody targets. This is the spine, so it lifts everything downstream. Effort: M to L. Dependency: avatar. Full spec in `Anvil_Segments_and_Angles_Spec.md`.
11. **Angle multiplication.** Turn one fact into many candidate hooks using the nine angle families (discovery, temporal, authority, conflict, narrative, analytical, social, emotional, problem-reframe). Extends the Curiosity engine, and the operator still judges which land. Hype-leaning families are capped for trust voices and the find-do-not-install rule is enforced. Effort: M. Dependency: Curiosity engine, segment builder. Full spec in `Anvil_Segments_and_Angles_Spec.md`.

### P2. Beyond the source: the bigger levers (from the pressure test)

12. **Close the loop.** Let the user paste back ad results so Anvil proposes the next test. This turns a generator into an optimizer and is the only feature here that compounds. Effort: M to L. Dependency: backend (P3) for storage. Highest strategic value.
13. **Offer-strength check.** Upgrade Step 0 to score and tighten the front-door offer against the value equation before any copy is written. Effort: M. Dependency: none.
14. **Lever checklist.** Add a short landing-page and speed-to-lead checklist plus a sharper creative brief, so the tool stops implying copy is the whole job. Effort: S. Dependency: none.
15. **Long-form mode (the advertorial).** A long-form, awareness-sequenced output for cold buyers, plus a short feeder ad that earns the click. It is the P1 depth pieces on a longer canvas. The most dangerous format for compliance, so it is gated hardest for trust markets and off by default for the heaviest tier. Effort: L. Dependency: Curiosity engine, Pain Chain and Promise Ladder builders, awareness stage. Full spec in `Anvil_Awareness_and_LongForm_Spec.md`.
16. **Structure library.** The 28 named ad and email templates rendered as safe block sequences, each tagged with awareness stage, channel, and claim risk. The user picks a structure or Anvil recommends one, then fills it with the operator's own compliant blocks, never the source's copy. Sharper after close-the-loop, which lets the library rank structures by what performed. Effort: M. Dependency: Curiosity engine, awareness stage. Full spec in `Anvil_Structure_Library_Spec.md`.
17. **Meta policy preflight.** Harden the compliance gate from a prompt instruction into a real rule check that screens generated copy against platform restricted-category and prohibited-claim rules before it reaches the human reviewer. This strengthens the P0 safety canon, which is the trust-market wedge. Effort: M. Dependency: compliance gate (P0), backend recommended.

### P3. Platform enablers

18. **Backend and web app.** A small serverless proxy holds the API key, removes the 1000-token artifact cap, and adds auth, rate limiting, and a usage table. Effort: M. Dependency: none, but it unblocks the quality ceiling for P1 and the storage for P2.
19. **Two-call avatar chain.** Split the avatar into two richer calls so it stops truncating and carries real depth. Effort: M. Dependency: backend recommended so each call has room.
20. **Review and forum mining.** Automatically source real buyer language from the client's own reviews and public forums to feed the segment builder, so the spine runs on real voice-of-customer instead of the operator supplying it by hand. Only the client's own material, within compliance. Effort: M to L. Dependency: backend, segment builder.
...
- The segment builder is the spine upgrade. Proof, Constraints, Curiosity, and the angle multiplication all read from it, so it raises the ceiling on all of P1. Review mining (P3 item 20) feeds it, and the two-call avatar chain (P3 item 19) carries it.
- Close the loop (P2 item 12) needs storage, so it follows the backend.
- The Meta policy preflight (P2 item 17) hardens the P0 compliance gate. It benefits from the backend but the rule check itself is the load-bearing part, not where it runs.

## 6a. Parked, not yet scoped

- **The create-manage seam.** Ad-ops agents exist that connect to Google, Meta, and LinkedIn and run the account after launch: search-terms cleanup, creative-fatigue watch, budget pacing, bid nudges. That is the manage side. Anvil is the create side. Nobody today connects the two, so a result from the manage side never feeds back into what Anvil generates next. Once close-the-loop (P2 item 12) ships, the natural extension is letting it draw on real launched-and-measured outcomes from a manage-side tool, not only Anvil's own history. This is a real moat because it spans both sides, but it depends on close-the-loop, on a stable integration point with a manage-side tool, and is not scoped further than this note. See `Anvil_Creative_Strategist_Positioning.md` for the full reasoning on the create and manage boundary.

## 7. One-paragraph recommendation

Do P0 this week. It is the cheapest, most honest work and it removes the two things that undercut the tool's credibility: a gamified metric the creators told you not to chase, and an unguarded hype dial in trust markets. Then decide the platform question. If Anvil is going to be a real rung in the Operator ladder, build the backend (P3) and use the freed token budget to port the depth in P1, which is the only path that separates Anvil from both ChatGPT and the framework's originators. The segment builder is the highest-value single upgrade in P1, since it is the spine everything else reads from. Keep P2 in view the whole time, because the loop and the offer check are where the actual money lever lives, above the copy.
